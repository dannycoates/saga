...
| `Elevator.js`            | **Elevator entity class**. Properties: position, velocity, destination, passengers, buttons, indicators. Methods: `tick()` for physics, `setDestinationFloor()`, `addPassenger()`, `removePassenger()`, `toAPI()` for player code, `toJSON()` for display.                                                                                                                                                      |
...
| `java.js`       | **Java runtime**. Loads CheerpJ from CDN. Precompiles base Elevator/Floor classes. Uses JNI callbacks (`Java_Elevator_jsSetDestinationFloor`) to bridge Java to JavaScript. Manages class versioning to avoid conflicts. |
