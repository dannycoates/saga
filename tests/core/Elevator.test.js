...
describe("Elevator class", () => {
  ...
  it("presses floor button and triggers events", () => {
    ...
    elevator.addEventListener("floor_buttons_changed", (e) =>
      buttonsChangedHandler(...e.detail),
    );
  ...
  it("removes user from elevator", () => {
    ...
    // Count users before
    const userCountBefore = elevator.userSlots.filter(
      (slot) => slot.user !== null,
    ).length;
    ...
    // Count users after
    const userCountAfter = elevator.userSlots.filter(
      (slot) => slot.user !== null,
    ).length;
  ...
  it("gets pressed floors correctly", () => {
    ...
    expect(elevator.pressedFloorButtons).toEqual([2, 5, 8]);
  });
  ...
  it("handles new state updates", () => {
    ...
    elevator.addEventListener("new_current_floor", (e) =>
      newFloorHandler(e.detail),
    );
