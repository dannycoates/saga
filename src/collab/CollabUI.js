...
export class CollabPanel extends HTMLElement {
    ...
    switch (this._state) {
      case "initial":
        return `
          ...
          <div class="actions">
            <button class="primary" data-action="host">Host Session</button>
          </div>
        `;
      ...
      case "join-from-link":
        return `
          ...
          <div class="actions">
            <button class="primary" data-action="join-from-link">Join Session</button>
          </div>
          ${this._error ? `<div class="error">${this._error}</div>` : ""}
        `;
        ...
        try {
          switch (action) {
            ...
            case "join-from-link": {
              ...
              const joinBtn = /** @type {HTMLButtonElement} */ (target);
  ...
  _createStatusIndicator() {
    ...
    el.innerHTML = `
      ...
      <span class="collab-peer-name"></span>
    ...
    `;
    el.querySelector(".layout-toggle-btn")?.addEventListener("click", () => {
      this.collabManager?.disconnect();
    });
  ...
  /**
   * Shows/hides the header status indicator based on connection state.
   * @private
   */
  _updateStatusIndicator() {
    ...
    if (this.collabManager?.connected) {
      ...
      const nameEl = this._statusEl.querySelector(".collab-peer-name");
