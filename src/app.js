...
class CodeEditor extends Observable {
  constructor(element, storageKey) {
    ...
    const defaultCode = document.getElementById('default-elev-implementation').textContent.trim();
  ...
  saveCode() {
    ...
    document.getElementById('save_message').textContent = 'Code saved ' + new Date().toTimeString();
    this.trigger('change');
  }
  ...
  setDevTestCode() {
    const devCode = document.getElementById('devtest-elev-implementation').textContent.trim();
...
export class ElevatorApp extends Observable {
  ...
  setupEventHandlers() {
    ...
    document.getElementById('button_resetundo').addEventListener('click', () => {
      if (confirm('Do you want to bring back the code as before the last reset?')) {
        this.editor.setCode(localStorage.getItem('develevateBackupCode') || '');
      }
      this.editor.view.focus();
    });
  ...
  loadFromUrl() {
    ...
    const timeScale = parseFloat(localStorage.getItem('elevatorTimeScale')) || 
