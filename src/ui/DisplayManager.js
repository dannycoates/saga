...
export class DisplayManager {
  ...
  cleanupExitedPassengers(currentPassengers) {
    ...
    // First pass: Clean up displays that are inactive
    ...
    // Second pass: Clean up displays for passengers no longer in simulation
    for (const [id, display] of this.passengerDisplays) {
      if (!currentIds.has(id)) {
        // Force DOM cleanup for passengers that have been removed from simulation
        display.dispatchEvent(new CustomEvent("removed"));
        this.passengerDisplays.delete(id);
      }
    }
  ...
  /**
   * Get display objects for presentation
   */
  getDisplays() {
    return {
      floors: this.floorDisplays,
      elevators: this.elevatorDisplays,
      passengers: this.passengerDisplays,
    };
  }
